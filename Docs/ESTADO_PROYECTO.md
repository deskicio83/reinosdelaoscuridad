...
- [x] S04b — Migración a Newtonsoft.Json
  - Reemplazado `JsonUtility.FromJson` por `JsonConvert.DeserializeObject` en PlayerDataSystem
  - `using Newtonsoft.Json` añadido a GlobalVariables · HeroCatalog · PlayerData
  - Validado: `awakenInventory.items` = 18 entradas correctas (caos_extracto=245, etc.)
...
- `RangoPM.max` es `int?` — Newtonsoft lo maneja nativamente (JsonUtility no podía)
- `GameManager` tiene `[DefaultExecutionOrder(-100)]` — inicializa antes que cualquier sistema
...
- Deserialización de JSONs: **Newtonsoft.Json** en todo el proyecto (`com.unity.nuget.newtonsoft-json`)
