...
_Última actualización: S25_tests — 2026-04-26_
...
| S25_tests | Batería completa de tests NUnit — 6 EditMode + 3 PlayMode. Assembly definitions separados. Correcciones de firmas respecto al spec (TryApplyEffect, RemoveEffect, danoTotal, LastContext setter). |
...
| 🟡 Media | RewardPanel + BootScene — pasos manuales pendientes | 1) `Tools → 8. Setup RewardPanel Prefab` (regenera prefab con 3 botones y nuevos anchors). 2) `Tools → 9. Setup BootScene` (añade CombatSystem GO a BootScene para que arranque con DDOL desde el primer frame). |
...
| S25_tests | ✓ Completado — Ver arriba | — |
| S26 | ArenaScene — PvP asincrónico | Media |
